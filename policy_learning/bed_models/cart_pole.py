...
class CartPole(BEDModel):
    """CartPole control problem e.g. https://arxiv.org/abs/2402.07868"""
    ...
    def __init__(
        ...
        dt: float = 0.05,
        stiffness_params: tuple | None = None,
        ...
        if not stiffness_params:
            self.stiffness_params = jnp.array([0.01, 0.01, 0.01, 0.01])
        else:
            self.stiffness_params = jnp.array(stiffness_params)
        ...
        self.design_bounds = jnp.array([5.0], dtype=self.dtype)
        self.design_bijector = lambda x: jnp.clip(x, -5.0, 5.0)
    ...
    def _noise_scale_col(self, noise_mult: Array | None, unbatched: bool) -> Array:
        """Per-trajectory noise multiplier as a ``[b, 1]`` column (or scalar 1.0).

        Used inside the EM scan, where the carry / noise terms are ``[b, d_y]``.
        ``None`` returns 1.0 (fixed-noise behaviour). For the unbatched call path
        a scalar lambda is promoted to a single-row ``[1, 1]`` column.
        """
        if noise_mult is None:
            return jnp.asarray(1.0, dtype=self.dtype)
        noise_mult = jnp.asarray(noise_mult, dtype=self.dtype)
        if unbatched:
            noise_mult = jnp.reshape(noise_mult, (1,))
        return noise_mult[:, None]
    ...
    def mean_dynamics_fn(self, y: Array, xi: Array, theta: Array) -> Array:
        ...
        mp = theta[..., 1]
        mc = theta[..., 2]
        k1 = self.stiffness_params[0]
        k2 = self.stiffness_params[1]
        d1 = self.stiffness_params[2]
        d2 = self.stiffness_params[3]

        s = y[..., 0]  # position of the cart
        q = y[..., 1]  # angle of the pole
        ds = y[..., 2]  # velocity of the cart
        dq = y[..., 3]  # angular velocity of the pole

        dds = (
            xi[..., 0]
            + mp * jnp.sin(q) * (l * dq**2 + self.grav_accel * jnp.cos(q))
            - (k1 * s + d1 * ds)
            - (k2 * q + d2 * dq) * jnp.cos(q) / l
        ) / (mc + mp * jnp.sin(q) ** 2)
        ddq = (
            -xi[..., 0] * jnp.cos(q)
            - mp * l * dq**2 * jnp.cos(q) * jnp.sin(q)
            - (mc + mp) * self.grav_accel * jnp.sin(q)
            - (k1 * s + d1 * ds) * jnp.cos(q)
            - (k2 * q + d2 * dq) * jnp.cos(q) ** 2 / l
        ) / (l * (mc + mp * jnp.sin(q) ** 2))

        drift_term = jnp.concatenate(
            [ds[..., None], dq[..., None], dds[..., None], ddq[..., None]], axis=-1
        )
    ...
    def sample_data(
        ...
        def EM_scan_fn(carry_state: Array, new_state: tuple[Array, Array, Array]):
            ...
            # noise_scale (per-trajectory lambda, broadcast [b, 1]) scales the
            # observation noise for NCSN annealing; 1.0 reproduces fixed noise.
            y_next = self.mean_dynamics_fn(y, xi, theta) + noise_scale * (
                jnp.sqrt(self.dt) * jnp.matmul(epsilon, self.diffusion_mat)
            )
        ...
        noise_scale = self._noise_scale_col(noise_mult, unbatched)
    ...
    def sample_designs(
        ...
        Args:
            ...
            sampler_type: Type of sampler to use ("uniform", "random_at_bounds",
                "pseudo_random", "temp_corr", "tanh_normal", "random_sampler").
    ...
    def data_reparam(
        self, epsilon: Array, xi: Array, theta: Array, noise_mult: Array | None = None
    ) -> tuple[Array, dict]:
        @check_shapes(
            "carry_state: [b, 4]",
            "new_state[0]: [b, 1]",
    ...
    def data_reparam_singleT(
        ...
        noise_scale = 1.0 if noise_mult is None else noise_mult[..., None]
        y_next = self.mean_dynamics_fn(y_prev, xi, theta) + noise_scale * (
            jnp.sqrt(self.dt) * jnp.matmul(epsilon, self.diffusion_mat)
        )
        return y_next
    ...
    @check_shapes("theta: [b, T, 3]", "return: [b]")
    def prior_log_lik(self, theta: Array) -> Array:
        theta = theta[:, 0, :]  # [b, T, 3] -> [b, 3]
    ...
    def per_step_data_log_lik(
        ...
        if noise_mult is None:
            ...
            # Scale the per-trajectory covariance by lambda**2 so the conditional
            # score becomes -(y - mu) / (lambda**2 sigma_base**2). The leading
            # [*batch, 1, d_y, d_y] cov broadcasts over the T axis of yT/means.
    ...
    def log_lik(
        ...
        return self.data_log_lik(
            yT, xT, theta, aux_data, noise_mult=noise_mult
        )  # + self.prior_log_lik(theta)

    @check_shapes("xi: [*batch, T, 1]", "return: [*batch]")
    def marginal_design_log_lik(self, xi: Array) -> Array:
        """Log-pdf of Uniform[-5, 5] design prior (constant log-density)."""
        xi = jnp.squeeze(xi, axis=-1)
        return jnp.sum(jnp.full_like(xi, jnp.log(1.0 / (2.0 * 5.0))), axis=-1)
