...
SAMPLER_WEIGHTS = {
    ...
    "pseudo_random_independent": 1.0,
    "pseudo_random_pos_correlated": 1.0,
    "pseudo_random_neg_correlated": 1.0,
    "temp_corr": 1.0,
    "tanh_normal": 0.0,
...
class DoublePendulum(BEDModel):
    """Stochastic Double-Link experiment from https://arxiv.org/abs/2402.07868"""
    ...
    def __init__(
        ...
        self.T = T
        self.d = 2  # Dimension of design space
        self.theta_dim = 4
        self.d_y = 4  # Dimension of observation space
        self.dim = T * (
            self.d + self.d_y
        ...
        if not diffusion_mat:
            self.diffusion_mat = jnp.diag(jnp.array([0.1, 0.1, 0.1, 0.1]))
        else:
        ...
        self.design_bounds = jnp.array([4.0, 2.0], dtype=self.dtype)
        self.design_bijector = lambda x: jnp.clip(
            x, -self.design_bounds, self.design_bounds
        )
    ...
    def mean_dynamics_fn(self, y: Array, xi: Array, theta: Array) -> Array:
        ...
        m1 = theta[..., 0]  # mass of first link
        m2 = theta[..., 1]
        l1 = theta[..., 2]  # length of first link
        l2 = theta[..., 3]

        q1 = y[..., 0]  # angle first link
        q2 = y[..., 1]  # angle second link
        dq1 = y[..., 2]  # angular velocity of first link
        dq2 = y[..., 3]  # angular velocity of second link
        dq = y[..., 2:]

        m11 = (m1 + m2) * l1**2 + m2 * l2**2 + 2 * m2 * l1 * l2 * jnp.cos(q2)
        m12 = m2 * l2**2 + m2 * l1 * l2 * jnp.cos(q2)
        m21 = m2 * l2**2 + m2 * l1 * l2 * jnp.cos(q2)
        m22 = m2 * l2**2
        row1 = jnp.stack([m11, m12], axis=-1)
        row2 = jnp.stack([m21, m22], axis=-1)
        M = jnp.stack([row1, row2], axis=-2)
        M_inv = jnp.linalg.inv(M)

        c12 = -m2 * l1 * l2 * jnp.sin(q2) * (2 * dq1 + dq2)
        c21 = 0.5 * m2 * l1 * l2 * (2 * dq1 + dq2) * jnp.sin(q2)
        c22 = -0.5 * m2 * l1 * l2 * dq1 * jnp.sin(q2)
        row1 = jnp.stack([jnp.zeros_like(c12), c12], axis=-1)
        row2 = jnp.stack([c21, c22], axis=-1)
        C = jnp.stack([row1, row2], axis=-2)

        t1 = (m1 + m2) * l1 * jnp.sin(q1) + m2 * l2 * jnp.sin(q1 + q2)
        t2 = m2 * l2 * jnp.sin(q1 + q2)
        Tau = -jnp.stack([t1, t2], axis=-1) * self.grav_accel

        # Messy shapes to deal with col vectors but essentially doing M_inv @ (Tau + xi - C @ dq)
        Cxdq = (C @ dq[..., None])[..., 0]  # [*batch, 2]
        B = Tau + xi - Cxdq  # [*batch, 2]
        dq_drift_term = M_inv @ B[..., None]  # [*batch, 2, 1]
        dq_drift_term = dq_drift_term[..., 0]  # [*batch, 2]

        drift_term = jnp.concatenate(
            [dq1[..., None], dq2[..., None], dq_drift_term], axis=-1
        )
    ...
    @check_shapes(
        "xi: [*batch_shape, T, 2]",
        "theta: [*batch_shape, T, 4]",
        "return[0]: [*batch_shape, T, 4]",
    )
    def sample_data(self, key: Key, xi: Array, theta: Array) -> tuple[Array, dict, Key]:
        ...
        xi = self.design_bijector(xi)  # [b, T, 2] map back to constrained space
        xs = (
            xi.transpose((1, 0, 2)),
        ...
        y = y.transpose((1, 0, 2))  # [b, T, 4]
        if unbatched:
            y = jnp.squeeze(y, axis=0)
            y_init = jnp.squeeze(y_init, axis=0)

        return y, {"y_init": y_init}, key
    ...
    def _sample_designs_gaussian(
        ...
        sigma_min1: float=0.5,
        sigma_max1: float=2.0,
        sigma_min2: float=0.2,
        sigma_max2: float=1.0,
        **kwargs,
    ) -> tuple[Array, dict, Key]:
        ...
        if self.d != 2:
            raise ValueError("sampler was meant for d=2")
        if scale is None:
            key, scale_key = jax.random.split(key)
            scale1 = jax.random.uniform(
                scale_key,
                (*batch_shape, 1, 1),
                minval=sigma_min1,
                maxval=sigma_max1,
        ...
        xi = normal_samples.at[..., 0].multiply(scale1).at[..., 1].multiply(scale2)
        return xi, {}, key
    ...
    def _sample_designs_random_at_bounds(
        ...
        **kwargs,
        ...
        saturation than toward the interior. xi = design_bounds * tanh(u).
        """
        if self._policy_parameterisation == "soft_constraint":
            raise ValueError("This sampler not meant for soft constraint")
        ...
        key, subkey_sign, subkey_jitter = jax.random.split(key, 3)
    ...
    def _sample_designs_pseudo_random_independent(
        self,
        key: Key,
        batch_shape: Shape,
        ...
        **kwargs,
        ...
        """Pseudo-random saturating designs with independent sign switches per dim."""
        if self._policy_parameterisation == "soft_constraint":
            raise ValueError("This sampler not meant for soft constraint")
    ...
    def _sample_designs_pseudo_random_pos_correlated(
        self,
        key: Key,
        batch_shape: Shape,
        switch_rate_min: float = 0.02,
        ...
        **kwargs,
        ...
        """Pseudo-random saturating designs with shared switches, equal signs across dims."""
        if self._policy_parameterisation == "soft_constraint":
            raise ValueError("This sampler not meant for soft constraint")
        if self.d != 2:
            raise ValueError("pseudo_random_pos_correlated requires 2D designs")
        if switch_rate_min < 0 or switch_rate_max < 0:
            raise ValueError("switch_rate bounds must be non-negative")
        if jitter_outward_std < 0 or jitter_inward_std < 0:
            raise ValueError("jitter stds must be non-negative")
        ...
        init_sign_shared = jnp.where(
            jax.random.bernoulli(subkey_init, p=0.5, shape=(*batch_shape, 1, 1)),
            jnp.array(1.0, dtype=self.dtype),
            jnp.array(-1.0, dtype=self.dtype),
        )
        init_sign = jnp.broadcast_to(init_sign_shared, (*batch_shape, 1, self.d))
        ...
        switch_prob = jnp.clip(1.0 - jnp.exp(-switch_rate), 0.0, 1.0)
        switches_shared = jax.random.bernoulli(
            subkey_switch,
            p=switch_prob,
            shape=(*batch_shape, max(self.T - 1, 0), 1),
        )
        switches = jnp.broadcast_to(
            switches_shared, (*batch_shape, max(self.T - 1, 0), self.d)
        )
        flip_factors = jnp.where(
            switches,
            -jnp.ones_like(switches, dtype=self.dtype),
    ...
    def _sample_designs_pseudo_random_neg_correlated(
        self,
        key: Key,
        batch_shape: Shape,
        switch_rate_min: float = 0.02,
        ...
        **kwargs,
        ...
        """Pseudo-random saturating designs with shared switches, opposite signs across dims."""
        if self._policy_parameterisation == "soft_constraint":
            raise ValueError("This sampler not meant for soft constraint")
        if self.d != 2:
            raise ValueError("pseudo_random_neg_correlated requires 2D designs")
        if switch_rate_min < 0 or switch_rate_max < 0:
            raise ValueError("switch_rate bounds must be non-negative")
        if jitter_outward_std < 0 or jitter_inward_std < 0:
            raise ValueError("jitter stds must be non-negative")
        ...
        init_sign_dim0 = jnp.where(
            jax.random.bernoulli(subkey_init, p=0.5, shape=(*batch_shape, 1, 1)),
            jnp.array(1.0, dtype=self.dtype),
            jnp.array(-1.0, dtype=self.dtype),
        )
        init_sign = jnp.concatenate([init_sign_dim0, -init_sign_dim0], axis=-1)
    ...
    def _sample_designs_temp_corr(
        ...
        **kwargs,
        ...
        Result stays in [-bounds, bounds] (convex combination). Marginals are
        approximately uniform for small rho. rho ~ 0.3 mimics early-policy
        temporal correlation; rho ~ 0.7 approaches pseudo_random-like behaviour.
        """
        if self._policy_parameterisation == "soft_constraint":
            raise ValueError("This sampler not meant for soft constraint")
        ...
        eps_t_first = jnp.moveaxis(eps, -2, 0)
        ...
        xi = jnp.moveaxis(xi_t_first, 0, -2)
        return xi, {}, key
    ...
    def _sample_designs_random_sampler(
        ...
        canonical_sampler_map = {
            ...
            "pseudo_random_independent": self._sample_designs_pseudo_random_independent,
            "pseudo_random_pos_correlated": self._sample_designs_pseudo_random_pos_correlated,
            "pseudo_random_neg_correlated": self._sample_designs_pseudo_random_neg_correlated,
    ...
    def sample_designs(
        ...
        Args:
            ...
            sampler_type: Type of sampler to use ("uniform", "random_at_bounds", "pseudo_random",
                "pseudo_random_independent", "pseudo_random_pos_correlated",
                "pseudo_random_neg_correlated", "temp_corr", "tanh_normal", "random_sampler").
                Defaults to "uniform" if None.
            sampler_kwargs: Keyword arguments to pass to the sampler.
                For pseudo-random samplers: {"switch_rate_min": float, "switch_rate_max": float,
                "jitter_scale": float}
                For "random_at_bounds": {"jitter_scale": float}
                For "random_sampler": optional keys like "candidate_samplers",
                "sampler_kwargs_by_type", and "sampler_weights_by_type".
        ...
        if sampler_type == "uniform":
            ...
            xi, aux, key = self._sample_designs_pseudo_random_independent(
                key, batch_shape, **sampler_kwargs
            )
        elif sampler_type == "pseudo_random_independent":
            xi, aux, key = self._sample_designs_pseudo_random_independent(
                key, batch_shape, **sampler_kwargs
            )
        elif sampler_type == "pseudo_random_pos_correlated":
            xi, aux, key = self._sample_designs_pseudo_random_pos_correlated(
                key, batch_shape, **sampler_kwargs
            )
        elif sampler_type == "pseudo_random_neg_correlated":
            xi, aux, key = self._sample_designs_pseudo_random_neg_correlated(
                key, batch_shape, **sampler_kwargs
            )
            ...
            raise ValueError(
                ...
                "{'uniform', 'random_at_bounds', 'pseudo_random', "
                "'pseudo_random_independent', 'pseudo_random_pos_correlated', "
                "'pseudo_random_neg_correlated', 'temp_corr', 'tanh_normal', "
                "'gaussian', 'fourier', 'random_sampler'}"
            )
    ...
    def sample_random_acquisition(
        ...
        if strategy == "random":  # uniform or beta if generalised
            ...
            raise NotImplementedError(f"Not implemented strategy {strategy} yet")
    ...
    def data_reparam(
        ...
        if unbatched:
            y = jnp.squeeze(y, axis=0)
            y_init = jnp.squeeze(y_init, axis=0)
        return y, {"y_init": y_init}  # [b, T, 4], y_init [b, 4]
    ...
    @check_shapes("theta: [b, T, 4]", "return: [b]")
    def prior_log_lik(self, theta: Array) -> Array:
        theta = theta[:, 0, :]  # [b, T, 4] -> [b, 4]
    ...
    @check_shapes("xi: [*batch, T, 2]", "return: [*batch]")
    def marginal_design_log_lik(self, xi: Array) -> Array:
        """Log-pdf of Uniform[-bounds, bounds] design prior (constant per-dimension log-density)."""
        log_density_per_dim = jnp.log(1.0 / (2.0 * self.design_bounds))  # [2]
        return jnp.sum(jnp.broadcast_to(log_density_per_dim, xi.shape), axis=(-1, -2))
